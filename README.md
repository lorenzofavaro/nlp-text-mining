...
Ontology learning represents the process of automatically creating an ontology to suffer from the texts and documents that are available. It can also be seen how the transition from an unstructured data, such as a text document, to a structured data in an automatic way. One of the different methodologies we have studied to do ontology learning is the Formal Concept Analysis. The latter represents a method that comes from the mathematical field, which allows you to automatically induce taxonomies starting from unstructured data. Fundamental to understand the FCA mechanism is the adjacency matrix, a matrix whose rows are represented by concepts and columns by features. The lattice constructed in the exercise is composed of 7 levels of depth, where each node represents a concept or a property norms and the arcs allow to relate each concept with the property norms associated with it. These concepts form a direct acyclic graph in which a hierarchy is imposed. In the upper part of the lattice there are the super concepts, that is the more general ones, while in the lower part the sub-concepts, that is the more specific concepts.
It is possible to navigate the lattice and retrieve the concepts through the basic functions of [concepts](https://pypi.org/project/concepts/) package. .
[graphviz](https://pypi.org/project/graphviz/) is used to print the graph.

By analyzing a specific domain of interest, for example that of fruit, concepts and features are obtained, such as: “apple” and “has peel”. The characteristics are modeled as Boolean functions that identify the presence or absence of a property norms in each concept. From this matrix the formal context is constructed, a taxonomic representation of the concepts and features in the form of a lattice, whose nodes are the latent concepts and the arcs are hyperonimic relations. The concepts of the matrix are merged into these new latent concepts and a taxonomic structure is derived. We proceeded by using functions from [concepts](https://pypi.org/project/concepts/), to carry out some experiments. For example, to find properties in common among several individuals of the population included in the dataset, or to verify all the individuals who have a certain property, or all individuals who share a certain property.
